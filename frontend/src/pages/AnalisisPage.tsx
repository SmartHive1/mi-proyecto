...
import { produccionMensual, comparativoData, cosechasData } from '../lib/mockData';
        ...
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={comparativoData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fill: '#80756A', fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey={selectedYear} fill="#F5A623" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-[#231710] border border-[#2A1B13] rounded-2xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {cosechasData.map((row, i) => (
                <tr key={i} className="border-b border-[#2A1B13]/50 hover:bg-[#2A1B13]/30 transition-colors duration-200">
                  <td className="px-6 py-4 text-[#FFF8ED] text-sm font-medium">{row.mes}</td>
                  <td className="px-6 py-4 text-[#80756A] text-sm">{row.colmenar}</td>
                  <td className="px-6 py-4 text-[#FFF8ED] text-sm font-semibold">{row.kg} kg</td>
                  <td className="px-6 py-4">
                    <span
                      className={`text-sm font-semibold ${
                        row.variacion >= 0 ? 'text-[#4CAF50]' : 'text-[#E53935]'
                      }`}
                    >
