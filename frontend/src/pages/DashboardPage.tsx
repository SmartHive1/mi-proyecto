...
import { colmenares, colmenas } from '../lib/mockData';

type Colmenar = typeof colmenares[0];

function ColmenarCard({ item, onClick }: { item: Colmenar; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
function ColmenaDetailItem({ colmena }: { colmena: typeof colmenas[0] }) {
  const nivelColor =
...
export default function DashboardPage() {
  const [selectedColmenar, setSelectedColmenar] = useState<Colmenar | null>(null);

  const colmenasDeSeleccionado = selectedColmenar
    ? colmenas.filter((c) => c.colmenarId === selectedColmenar.id)
    : [];
  ...
  if (selectedColmenar) {
    return (
      <div>
        <button
          onClick={() => setSelectedColmenar(null)}
        ...
        <div className="mb-6">
          <h1 className="text-[#FFF8ED] text-2xl font-bold">{selectedColmenar.nombre}</h1>
          <div className="flex items-center gap-1.5 text-[#80756A] text-sm mt-1">
            <MapPin size={13} />
            <span>{selectedColmenar.ubicacion}</span>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {colmenasDeSeleccionado.map((c) => (
            <ColmenaDetailItem key={c.id} colmena={c} />
          ))}
        </div>

        {colmenasDeSeleccionado.length === 0 && (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Colmenares', value: colmenares.length },
          { label: 'Total colmenas', value: colmenas.length },
          { label: 'Alertas activas', value: 2, highlight: true },
          { label: 'Sensores online', value: `${colmenas.length - 1}/${colmenas.length}` },
        ].map(({ label, value, highlight }) => (
          <div key={label} className="bg-[#231710] border border-[#2A1B13] rounded-xl p-4">
            <p className="text-[#80756A] text-xs mb-1">{label}</p>
            <p
              className={`text-2xl font-bold ${highlight ? 'text-[#E53935]' : 'text-[#FFF8ED]'}`}
            >
              {value}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {colmenares.map((item) => (
          <ColmenarCard key={item.id} item={item} onClick={() => setSelectedColmenar(item)} />
        ))}
      </div>
    </div>
  );
