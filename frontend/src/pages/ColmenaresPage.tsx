...
export default function ColmenaresPage() {
  const [selectedId, setSelectedId] = useState(colmenares[0].id);
  const selectedColmenar = colmenares.find((c) => c.id === selectedId)!;
  ...
  return (
    <div>
      ...
      {/* Selected colmenar info */}
      <div className="bg-[#231710] border border-[#2A1B13] rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-[#FFF8ED] font-semibold text-lg">{selectedColmenar.nombre}</h2>
            <p className="text-[#80756A] text-sm mt-0.5">{selectedColmenar.ubicacion}</p>
          </div>
          <span className="text-[#F5A623] text-sm font-semibold bg-[#F5A623]/10 px-3 py-1.5 rounded-xl">
            {selectedColmenar.cantidadColmenas} colmenas
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {colmenasDelSeleccionado.map((colmena) => {
          ...
          return (
            <div
              key={colmena.id}
              className="bg-[#231710] border border-[#2A1B13] rounded-xl p-5"
            >
