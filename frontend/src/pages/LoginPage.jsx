...
function LoginPage() {
  ...
  const { iniciarTransicion } = useTransicion()
  ...
  async function handleSubmit(e) {
    ...
    if (email === USUARIO_TEST.email && password === USUARIO_TEST.password) {
      ...
      // El callback se ejecuta cuando la pantalla está llena de hexes
      // → navega, el dashboard se monta detrás, luego los hexes se van
      iniciarTransicion(() => navigate('/dashboard'))
    } else {
  ...
  return (
    <div className="flex h-screen w-full bg-[#1A120B] overflow-hidden relative">
      {/* Panel izquierdo */}
      <div
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-[#FFF8ED] text-4xl font-bold leading-tight mb-4">
            Apicultura de<br /><span className="text-[#F5A623]">precisión</span>
          </h1>
        ...
        <div className="w-full max-w-md">
          <div className="bg-[#231710] rounded-2xl p-8 sm:p-10 border border-[#2A1B13]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-[#80756A] text-xs font-semibold uppercase tracking-widest mb-2">Correo electrónico</label>
              ...
              <div>
                <label className="block text-[#80756A] text-xs font-semibold uppercase tracking-widest mb-2">Contraseña</label>
                <div className="relative">
                  <Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#80756A]" />
                  <input type={mostrarPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" className="w-4 h-4 rounded border-[#3a2a1e] bg-[#2A1B13] accent-[#F5A623]" />
                  <span className="text-[#80756A] text-sm">Recordarme</span>
                </label>
                <button type="button" className="text-[#F5A623] text-sm hover:opacity-80 transition-opacity duration-200">
                  ¿Olvidaste tu contraseña?
                </button>
              </div>
              <button type="submit" disabled={cargando || transicionando}
